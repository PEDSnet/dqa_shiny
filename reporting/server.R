...
shinyServer(function(input, output) {
  ...
  vc_output <- reactive({
    if(input$largen_toggle==1){
      res('vc_vs_output_merge_pp') %>% filter(check_type=='vc')%>%
        mutate(prop_viol=round(tot_prop,2))
    }else{res('vc_vs_output_merge_ln')%>%filter(check_type=='vc')}
  })
  ...
  vs_output <- reactive({
    if(input$largen_toggle==1){
      res('vc_vs_output_merge_pp') %>% filter(check_type=='vs')%>%
        mutate(prop_viol=round(tot_prop,2))
    }else{res('vc_vs_output_merge_ln') %>%filter(check_type=='vs')}
  })

  vc_vs_violations <- reactive({
    res('vc_vs_violations_merge_pp') %>%
      mutate(prop_viol=round(tot_prop,2))
  })
  ### adjust available site name
  ...
  uc_output <- reactive({
    if(input$largen_toggle==1){
      res('uc_output_merge_pp')
    }else{
      res('uc_output_merge_ln')
    }
  })
  uc_yr_output <- reactive({
    if(input$largen_toggle==1){
      res('uc_by_year_merge_pp') %>%
        mutate(year_date=as.integer(year_date))
    }else{
      res('uc_by_year_merge_ln')%>%
        mutate(year_date=as.integer(year_date))
    }
  })
  uc_top_output <- reactive({
    results_tbl('uc_grpd_merge_pp') %>% collect()%>%
  ...
  pf_output <- reactive({
    if(input$largen_toggle==1){
      res('pf_output_merge_pp')
    }else{res('pf_output_merge_ln')}
  })
  ### adjust available site name
  ...
  ### fetch data
  top_rolled <-  results_tbl('bmc_gen_output_concepts_merge_pp')%>%
  ...
  bmc_pp <- reactive({
    if(input$largen_toggle==1){
      res('bmc_gen_output_merge_pp')%>%
        left_join(top_rolled, by = c('site', 'check_desc'))
    }else{
      res('bmc_gen_output_merge_ln')%>%
        left_join(top_rolled, by = c('site', 'check_desc'))
    }
  })
  bmc_pp_concepts <- reactive({
    res('bmc_gen_output_concepts_merge_pp')
  })
  # adjust available site name
  observeEvent(bmc_pp(), {
  ...
  bmc_conceptset <- results_tbl('bmc_conceptset_merge')%>%filter(!is.na(include))%>%collect()
  ...
  # facts over time ------
  # capture data
  fot_output_summary_ratio <- reactive({res('fot_output_mnth_ratio_merge_pp')})
  ...
  fot_output_heuristic <- reactive({results_tbl('fot_heuristic_summary_merge_pp') %>%
  ...
  fot_output <- reactive({res('fot_heuristic_merge_pp') %>%
      inner_join(res('fot_heuristic_summary_merge_pp'),
                 by=c('domain','check_name', 'site','site_anon', 'sitenum')) %>%
      inner_join(select(res('fot_output_mnth_ratio_merge_pp'),
                        c(row_cts, check_name, domain, site, month_end)),
                 by = c('check_name', 'domain', 'site', 'month_end'))%>%
      collect() %>%
      filter(domain==input$fot_domain)
  })
  fot_output_site <- reactive({res('fot_heuristic_merge_pp') %>%
      inner_join(res('fot_heuristic_summary_merge_pp'),
                 by=c('domain','check_name', 'site','site_anon', 'sitenum')) %>%
      inner_join(select(res('fot_output_mnth_ratio_merge_pp'),
                        c(row_cts, check_name, domain, site, month_end)),
  ...
  dcon_output <- reactive({
    if(input$largen_toggle==1){
      indat<-res('dcon_output_merge_pp')
    }else{
      indat<-res('dcon_output_merge_ln')
    }
    indat%>%
  ...
  mf_visitid_output <- reactive({
    if(input$largen_toggle==1){
      res('mf_visitid_merge_pp')
    }else{
      res('mf_visitid_merge_ln')
    }
  })
  ...
  ecp_output <- reactive({
    if(input$largen_toggle==1){res('ecp_output_merge_pp')}else{res('ecp_output_merge_ln')}
  })
  # update choices for site name
